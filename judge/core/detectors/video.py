...
class VideoAnomalyDetector(BaseDetector):
    ...
    def detect(self, file_path: Path, metadata: Optional[Dict] = None) -> List[AnomalyEvent]:
        ...
        while True:
            ...
            # Frame-to-frame intensity change
            ...
            edge_density = float(np.mean(np.hypot(sobelx, sobely) > 80))
        ...
        # Threshold driven by sensitivity
        thresh = self._threshold(5.5)   # tuned for MAD scale
    ...
    def _group_events(
        ...
        n = len(t)
        i = 0
        min_len_samples = max(1, int(self.min_duration * fps / self.sample_every))
