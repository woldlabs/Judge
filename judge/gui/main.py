...
class JudgeApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # Sensitivity
        self.sens_var = ctk.DoubleVar(value=0.65)
        ...
        self.sens_label = ctk.CTkLabel(sens_frame, text="0.65", font=ctk.CTkFont(size=10))
        ...
        self.min_dur_var = ctk.DoubleVar(value=0.05)
        ...
        self.progress = ctk.CTkProgressBar(bottom, width=520)
        self.progress.pack(side="left", padx=16, pady=14)
        self.progress.set(0)

        self.status_label = ctk.CTkLabel(bottom, text="Idle", font=ctk.CTkFont(size=10))
        self.status_label.pack(side="left", padx=8)

        self.log_box = ctk.CTkTextbox(bottom, height=58, width=480, font=ctk.CTkFont(family="Consolas", size=9))
    ...
    def _on_run_analysis(self):
        if self.session is None or len(self.session.files) == 0:
            messagebox.showwarning("No data", "Please add files or a folder first.")
            return

        # Rebuild session with current UI params
        ...
        for f in list(self.session.files):  # re-add current files
            pass  # files were already added to the old session; rebuild
        # Actually re-add files from previous session state
        # We need to persist the list of paths
        # Simpler: keep file list in app and rebuild
        if not hasattr(self, "_current_paths"):
            self._current_paths = [str(p) for p in (self.session.files if self.session else [])]
        self.session.files = []
        for p in getattr(self, "_current_paths", []):
    ...
    def _update_progress(self, msg: str, pct: float):
        ...
        self.status_label.configure(text=f"{msg} ({pct:.0f}%)")

    def _analysis_finished(self, result: AnalysisResult):
        self.result = result
        self.events = result.events
        self._current_paths = result.files_processed[:]
