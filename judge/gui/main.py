...
class JudgeApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.hail_mary_btn = ctk.CTkButton(btn_frame, text="Hail Mary", width=85, height=36,
                                           command=self._on_hail_mary, state="disabled")
        self.hail_mary_btn.pack(side="left", padx=4)

        # Main area - use grid for better resizing of center (visuals)
        ...
        ctk.CTkButton(btns, text="+ Add Files", command=self._on_add_files, width=220).pack(fill="x", pady=2)
        ctk.CTkButton(btns, text="+ Add Folder", command=self._on_add_folder, width=220).pack(fill="x", pady=2)
        ctk.CTkButton(btns, text="Remove File", fg_color="#5c2a2a", hover_color="#3f1f1f",
                      command=self._on_remove_file, width=220).pack(fill="x", pady=2)
        ctk.CTkButton(btns, text="Clear All", fg_color="#5c2a2a", hover_color="#3f1f1f",
                      command=self._on_clear_files, width=220).pack(fill="x", pady=2)
        ...
        # Center - visualizations + events (expands on resize)
        ...
        right.grid(row=0, column=2, sticky="ns", padx=(4, 0), pady=4)
        right.pack_propagate(False)

        ctk.CTkLabel(right, text="EVENT DETAILS", font=ctk.CTkFont(size=13, weight="bold")).pack(padx=10, pady=(10, 4), anchor="w")
        ...
        # Bigger % + main message
        self.pct_label = ctk.CTkLabel(bottom, text="0%", font=ctk.CTkFont(size=16, weight="bold"), width=50)
        self.pct_label.pack(side="left", padx=(0, 4))

        self.status_label = ctk.CTkLabel(bottom, text="Idle", font=ctk.CTkFont(size=14))
        self.status_label.pack(side="left", padx=4)

        # Short live activity area (prevents "frozen" feeling on large files)
        self.activity_label = ctk.CTkLabel(bottom, text="", font=ctk.CTkFont(size=11), text_color="#aaa")
        self.activity_label.pack(side="left", padx=8)

        # Time remaining estimate
        self.eta_label = ctk.CTkLabel(bottom, text="", font=ctk.CTkFont(size=11), text_color="#888")
        ...
        # Footer
        footer = ctk.CTkLabel(self, text="Created with care by Wold Labs, the truth is out there", font=ctk.CTkFont(size=8), text_color="#666")
    ...
    def _update_file_list(self):
        ...
        for fp in paths:
            ...
            name = Path(fp).name
            lbl = ctk.CTkLabel(item_frame, text=f"• {name}", font=ctk.CTkFont(size=10), anchor="w")
    ...
    def _on_add_files(self):
        ...
        self.hail_mary_btn.configure(state="normal" if self._file_paths else "disabled")
    ...
    def _on_remove_file(self):
        if not self._file_paths:
            return
        # Remove last as simple; per-item X buttons allow specific
    ...
    def _on_run_analysis(self):
        ...
        def worker():
            try:
                res = self.session.run(cancel_event=self.cancel_event, pause_event=self.pause_event)
    ...
    def _toggle_pause(self):
        ...
        if self.pause_event.is_set():
            ...
            self._log("Resumed analysis.")
            self._update_activity("resumed detectors")
        else:
            self.pause_event.set()
            self.run_btn.configure(text="▶ RESUME")
            self._log("Paused analysis.")
            self._update_activity("paused (click to resume)")

    def _on_hail_mary(self):
        if not self._file_paths:
            messagebox.showwarning("No data", "Please add files or a folder first.")
            return
    ...
    def _render_event_list(self):
        ...
        for ev in sorted(self.events, key=lambda e: -e.score):
            ...
            ctk.CTkLabel(card, text=header, font=ctk.CTkFont(size=13, weight="bold"),
                         text_color="#dfe3ff").pack(anchor="w", padx=8, pady=(5, 0))

            short = ev.description[:140] + ("..." if len(ev.description) > 140 else "")
            ctk.CTkLabel(card, text=short, font=ctk.CTkFont(size=11), wraplength=640,
